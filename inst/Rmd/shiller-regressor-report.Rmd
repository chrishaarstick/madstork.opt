...
df <- clean %>% 
  dplyr::mutate(ftr_ret_1yr = lead(price, 12) / price - 1,
                ...
                gs10_d_1yr = gs10 / lag(gs10, 12) - 1,
                gs10_d_5yr = gs10 / lag(gs10, 60) - 1,
...
feature_pipe <- pipeline(expr = function(df) {
  ...
  df <- df %>% 
    dplyr::select(ftr_ret_1yr, ret_1yr, ret_2yr, ret_5yr, ret_10yr,
                  ...
                  gs10_real, gs10_d_1yr, gs10_d_5yr, div_yield_real)  %>% 
...
```{r execution, eval=TRUE, ref.label=c('r-config', 'r-setup', 'spark-config', 'spark-setup',  'data-import', 'data-prep', 'data-transform', 'data-split','spark-import', 'feature-pipeline', 'regressor', 'predictions')}
...
* Current predictions written to file here: _`r print(current_pred_path)`_
